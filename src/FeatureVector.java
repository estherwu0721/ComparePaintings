...
public class FeatureVector {
	...
	public FeatureVector(int bpp) {
		keySpaceSize = (int) (Math.pow(2, bpp));
	...
	public void getTheCounts(ColorHash ch) {
		...
		for (int i = 0; i < ch.getTableSize(); i++) {
			if (ch.getKeyAt(i) != null) {
				int originalIndex = ch.getKeyAt(i).hashCode();
				colorCounts[originalIndex] = ch.getValueAt(i);
			}
		}
	}
	public double cosineSimilarity(FeatureVector other) {
		...
		for (int i = 0; i < keySpaceSize; i++) {
			long A = this.colorCounts[i];
			long B = other.colorCounts[i];
			if (A != -1 && B != -1) {
				top += (double)A * (double)B;
				bottomOne += (double)A * (double)A;
				bottomTwo += (double)B * (double)B;
			}
		}
		double totalSimilarity = top / (Math.sqrt(bottomOne) * Math.sqrt(bottomTwo));
		return totalSimilarity; // Change this to return the actual value.
	}
